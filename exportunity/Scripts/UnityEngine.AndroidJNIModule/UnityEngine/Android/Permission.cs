...
public struct Permission
{
	...
	public static bool HasUserAuthorizedPermission(string permission)
	{
		...
		return GetUnityPermissions().CallStatic<bool>("hasUserAuthorizedPermission", new object[2]
		{
			Common.GetActivity(),
			permission
		});
	...
	public static void RequestUserPermissions(string[] permissions, PermissionCallbacks callbacks)
	{
		if (permissions != null && permissions.Length != 0)
		{
			GetUnityPermissions().CallStatic("requestUserPermissions", Common.GetActivity(), permissions, callbacks);
		}
	}
}
