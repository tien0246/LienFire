...
public class XmlSchemaImporter : SchemaImporter
{
	...
	internal class ElementComparer : IComparer
	{
		...
		{
			ElementAccessor obj = (ElementAccessor)o1;
			return string.Compare(strB: ((ElementAccessor)o2).ToString(string.Empty), strA: obj.ToString(string.Empty), comparisonType: StringComparison.Ordinal);
		}
	}
	...
	{
		PrimitiveMapping obj = new PrimitiveMapping
		{
			TypeDesc = base.Scope.GetTypeDesc("string", "http://www.w3.org/2001/XMLSchema", flags)
		};
		obj.TypeName = obj.TypeDesc.DataType.Name;
		obj.Namespace = "http://www.w3.org/2001/XMLSchema";
		return obj;
	}
	...
	public XmlTypeMapping ImportDerivedTypeMapping(XmlQualifiedName name, Type baseType, bool baseTypeCanBeIndirect)
	{
		...
		else if (baseType != null)
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("Element '{0}' from namespace '{1}' is not a complex type and cannot be used as a {2}.", name.Name, name.Namespace, baseType.FullName));
			}
	...
	public XmlTypeMapping ImportSchemaType(XmlQualifiedName typeName, Type baseType, bool baseTypeCanBeIndirect)
	{
		...
		else if (baseType != null)
		{
			if (!(elementAccessor.Mapping is ArrayMapping))
			{
				throw new InvalidOperationException(Res.GetString("Type '{0}' from namespace '{1}' is not a complex type and cannot be used as a {2}.", typeName.Name, typeName.Namespace, baseType.FullName));
			}
			elementAccessor.Mapping = ((ArrayMapping)elementAccessor.Mapping).TopLevelMapping;
			MakeDerived((StructMapping)elementAccessor.Mapping, baseType, baseTypeCanBeIndirect);
		}
	...
	{
		MembersMapping membersMapping = ImportType(typeName, typeof(MembersMapping), null, TypeFlags.CanBeElementValue, addref: true) as MembersMapping;
	...
	private TypeMapping ImportElementType(XmlSchemaElement element, string identifier, Type desiredMappingType, Type baseType, string ns)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("The element, {0}, from namespace, {1}, was imported in two different contexts: ({2}, {3}).", element.Name, ns, typeMapping.GetType().Name, desiredMappingType.Name));
		}
	...
	private void RunSchemaExtensions(TypeMapping mapping, XmlQualifiedName qname, XmlSchemaType type, XmlSchemaObject context, TypeFlags flags)
	{
		...
		catch (ArgumentException)
		{
			if (qname.IsEmpty)
			{
				throw new InvalidOperationException(Res.GetString("Schema importer extension {0} returned invalid type information: '{1}' is not a valid type name.", extension.GetType().FullName, text));
			}
			throw new InvalidOperationException(Res.GetString("Schema importer extension {0} returned invalid type information for xsd type {1} from namespace='{2}': '{3}' is not a valid type name.", extension.GetType().FullName, qname.Name, qname.Namespace, text));
		}
	...
	private TypeMapping ImportType(XmlQualifiedName name, Type desiredMappingType, Type baseType, TypeFlags flags, bool addref)
	{
		...
		if (addref)
		{
			AddReference(name, base.TypesInUse, "Type '{0}' from targetNamespace='{1}' has invalid definition: Circular type reference.");
		}
	...
	private StructMapping ImportStructType(XmlSchemaType type, string typeNs, string identifier, Type baseType, bool arrayLike)
	{
		...
		if (mapping != null)
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("The type '{0}' from namespace '{1}' was used in two different ways.", type.QualifiedName.Name, type.QualifiedName.Namespace));
			}
		...
		for (int i = 0; i < structMapping.Members.Length; i++)
		{
			...
			{
				throw new InvalidOperationException(Res.GetString("Error: Type '{0}' could not be imported because it redefines inherited member '{1}' with a different type. '{1}' is declared as type '{3}' on '{0}', but as type '{2}' on base class '{4}'.", type.Name, memberMapping.Name, memberMapping.TypeDesc.FullName, structMapping.Members[i].TypeDesc.FullName, declaringMapping.TypeDesc.FullName));
			}
		}
	...
	private MemberMapping ImportChoiceGroup(XmlSchemaGroupBase group, string identifier, CodeIdentifiers members, CodeIdentifiers membersScope, INameScope elementsScope, string ns, bool groupRepeats, ref bool needExplicitOrder, bool allowDuplicates)
	{
		...
		if (flag)
		{
			...
			if (membersScope != null)
			{
				string text = (memberMapping.ChoiceIdentifier.MemberName = membersScope.AddUnique(memberMapping.ChoiceIdentifier.MemberName, memberMapping2));
				string name = (memberMapping2.Name = text);
	...
	private bool GatherGroupChoices(XmlSchemaParticle particle, NameTable choiceElements, string identifier, string ns, ref bool needExplicitOrder, bool allowDuplicates)
	{
		if (particle is XmlSchemaGroupRef)
		{
			...
			{
				AddReference(xmlSchemaGroupRef.RefName, base.GroupsInUse, "Group '{0}' from targetNamespace='{1}' has invalid definition: Circular group reference.");
	...
	private void AddScopeElement(INameScope scope, ElementAccessor element, ref bool duplicateElements, bool allowDuplicates)
	{
		...
		if (elementAccessor != null)
		{
			if (!allowDuplicates)
			{
				throw new InvalidOperationException(Res.GetString("The XML element named '{0}' from namespace '{1}' is already present in the current scope.", element.Name, element.Namespace));
			}
			if (elementAccessor.Mapping.TypeDesc != element.Mapping.TypeDesc)
			{
				throw new InvalidOperationException(Res.GetString("The XML element named '{0}' from namespace '{1}' is already present in the current scope. Elements with the same name in the same scope must have the same type.", element.Name, element.Namespace));
			}
	...
	private TypeMapping ImportDataType(XmlSchemaSimpleType dataType, string typeNs, string identifier, Type baseType, TypeFlags flags, bool isList)
	{
		...
		if (dataType.Content is XmlSchemaSimpleTypeRestriction)
		{
			...
			AddReference(xmlSchemaSimpleTypeRestriction.BaseTypeName, base.TypesInUse, "Type '{0}' from targetNamespace='{1}' has invalid definition: Circular type reference.");
	...
	private PrimitiveMapping ImportPrimitiveDataType(XmlSchemaSimpleType dataType, TypeFlags flags)
	{
		...
		obj.Namespace = (obj.TypeDesc.IsXsdType ? "http://www.w3.org/2001/XMLSchema" : "http://microsoft.com/wsdl/types/");
		return obj;
	}
	...
	private XmlSchemaGroup FindGroup(XmlQualifiedName name)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("Group {0} is missing.", name.Name));
		}
	...
	private XmlSchemaAttributeGroup FindAttributeGroup(XmlQualifiedName name)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("The attribute group {0} is missing.", name.Name));
		}
	...
	private TypeDesc GetDataTypeSource(XmlSchemaSimpleType dataType, TypeFlags flags)
	{
		...
		AddReference(xmlQualifiedName, base.TypesInUse, "Type '{0}' from targetNamespace='{1}' has invalid definition: Circular type reference.");
	...
	private XmlSchemaSimpleType FindDataType(XmlQualifiedName name, TypeFlags flags)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("Referenced type '{0}' is only valid for encoded SOAP.", name.ToString()));
		}
	...
	private XmlSchemaElement FindElement(XmlQualifiedName name)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("The element '{0}' is missing.", name.ToString()));
		}
	...
	private XmlSchemaAttribute FindAttribute(XmlQualifiedName name)
	{
		...
		{
			throw new InvalidOperationException(Res.GetString("The attribute {0} is missing.", name.Name));
		}
