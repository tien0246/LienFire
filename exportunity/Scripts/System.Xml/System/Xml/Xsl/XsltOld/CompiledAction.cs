...
internal abstract class CompiledAction : Action
{
	...
	public void CompileAttributes(Compiler compiler)
	{
		...
		do
		{
			...
			try
			{
				...
				{
					throw XsltException.Create("'{0}' is an invalid attribute for the '{1}' element.", input.LocalName, localName);
				}
			}
			catch
			{
	...
	public void CheckEmpty(Compiler compiler)
	{
		...
		do
		{
			...
			{
				throw XsltException.Create("The contents of '{0}' must be empty.", name);
			}
		}
	...
	public void CheckRequiredAttribute(Compiler compiler, bool attr, string attrName)
	{
		if (!attr)
		{
			throw XsltException.Create("Missing mandatory attribute '{0}'.", attrName);
		}
	}
}
