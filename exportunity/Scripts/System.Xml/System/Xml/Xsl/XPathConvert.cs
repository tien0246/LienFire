	...
	private class FloatingDecimal
	{
		...
		private double AdjustDbl(double dbl)
		{
			...
			if (num5 > num3)
			{
				...
				num3 = 0;
				int num9 = 0;
				while (num5 >= 32 && bigInteger[num9] == 0)
				{
					num5 -= 32;
					num9++;
				}
				...
				for (num9 = 0; num9 < num5 && (num10 & (1L << num9)) == 0L; num9++)
				{
				}
				if (num9 > 0)
				{
			...
			if (num4 > 0)
			{
				bigInteger2.MulPow5(num4);
			}
			else if (num2 > 0)
			{
				bigInteger.MulPow5(num2);
			}
			if (num5 > 0)
			{
				bigInteger2.ShiftLeft(num5);
			}
			else if (num3 > 0)
			{
				bigInteger.ShiftLeft(num3);
			}
