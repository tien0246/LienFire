...
public static class XsltFunctions
{
	...
	public static double MSStringCompare(string s1, string s2, string lang, string options)
	{
		...
		if (flag)
		{
			...
			{
				throw new XslTransformException("String comparison option(s) '{0}' are either invalid or cannot be used together.", options);
			}
			compareOptions = CompareOptions.IgnoreCase;
		}
	...
	{
		if (ValidateNames.ParseQName(name, 0, out var colonOffset) != name.Length)
		{
			return string.Empty;
		}
		if (colonOffset == 0)
