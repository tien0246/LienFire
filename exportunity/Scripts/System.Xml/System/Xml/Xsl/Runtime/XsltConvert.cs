...
public static class XsltConvert
{
	...
	public static XPathNavigator ToNode(IList<XPathItem> listItems)
	{
		...
		throw new XslTransformException("Cannot convert a node-set which contains zero nodes or more than one node to a single node.", string.Empty);
	}
	...
	public static IList<XPathNavigator> EnsureNodeSet(IList<XPathItem> listItems)
	{
		if (listItems.Count == 1)
		{
			...
			if (!xPathItem.IsNode)
			{
				throw new XslTransformException("Expression must evaluate to a node-set.", string.Empty);
			}
			if (xPathItem is RtfNavigator)
			{
				throw new XslTransformException("To use a result tree fragment in a path expression, first convert it to a node-set using the msxsl:node-set() function.", string.Empty);
			}
		}
		return XmlILStorageConverter.ItemsToNavigators(listItems);
	}
