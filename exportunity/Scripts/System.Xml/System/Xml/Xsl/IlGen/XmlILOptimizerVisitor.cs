...
internal class XmlILOptimizerVisitor : QilPatternVisitor
{
	...
	protected override QilNode VisitPositionOf(QilUnary local0)
	{
		...
		if (this[XmlILOptimization.EliminatePositionOf] && qilNode.NodeType == QilNodeType.For && qilNode[0].XmlType.IsSingleton && AllowReplace(XmlILOptimization.EliminatePositionOf, local0))
		{
			return Replace(XmlILOptimization.EliminatePositionOf, local0, VisitLiteralInt32(f.LiteralInt32(1)));
		}
	...
	protected override QilNode VisitEq(QilBinary local0)
	{
			...
			if (qilNode8.XmlType.IsSingleton && qilNode2.NodeType == QilNodeType.StrConcat)
			{
				...
				if (qilNode9.NodeType == QilNodeType.Loop)
				{
					...
					if (qilNode10.NodeType == QilNodeType.For)
					{
						...
						if (!qilNode12.XmlType.MaybeMany && qilNode11.NodeType == QilNodeType.XsltGenerateId && qilNode11[0] == qilNode10 && AllowReplace(XmlILOptimization.NormalizeIdEq, local0))
						{
							QilNode qilNode13 = VisitFor(f.For(qilNode12));
							return Replace(XmlILOptimization.NormalizeIdEq, local0, VisitNot(f.Not(VisitIsEmpty(f.IsEmpty(VisitFilter(f.Filter(qilNode13, VisitIs(f.Is(qilNode8, qilNode13)))))))));
						}
					}
				}
			}
			...
			if (qilNode14.NodeType == QilNodeType.Loop)
			{
				...
				if (qilNode15.NodeType == QilNodeType.For)
				{
					...
					if (!qilNode17.XmlType.MaybeMany && qilNode16.NodeType == QilNodeType.XsltGenerateId && qilNode16[0] == qilNode15 && qilNode2.NodeType == QilNodeType.XsltGenerateId)
					{
						...
						if (qilNode18.XmlType.IsSingleton && AllowReplace(XmlILOptimization.NormalizeIdEq, local0))
						{
							QilNode qilNode19 = VisitFor(f.For(qilNode17));
							return Replace(XmlILOptimization.NormalizeIdEq, local0, VisitNot(f.Not(VisitIsEmpty(f.IsEmpty(VisitFilter(f.Filter(qilNode19, VisitIs(f.Is(qilNode18, qilNode19)))))))));
						}
					}
				}
			}
		...
		if (this[XmlILOptimization.NormalizeMuenchian] && qilNode.NodeType == QilNodeType.Length)
		{
			...
			if (qilNode20.NodeType == QilNodeType.Union)
			{
				...
				if (qilNode21.XmlType.IsSingleton && !qilNode22.XmlType.MaybeMany && qilNode2.NodeType == QilNodeType.LiteralInt32 && (int)((QilLiteral)qilNode2).Value == 1 && AllowReplace(XmlILOptimization.NormalizeMuenchian, local0))
				{
					QilNode qilNode23 = VisitFor(f.For(qilNode22));
					return Replace(XmlILOptimization.NormalizeMuenchian, local0, VisitIsEmpty(f.IsEmpty(VisitFilter(f.Filter(qilNode23, VisitNot(f.Not(VisitIs(f.Is(qilNode21, qilNode23)))))))));
				}
			}
		}
		if (this[XmlILOptimization.NormalizeMuenchian] && qilNode.NodeType == QilNodeType.Length)
		{
			...
			if (qilNode24.NodeType == QilNodeType.Union)
			{
				...
				if (!qilNode25.XmlType.MaybeMany && qilNode26.XmlType.IsSingleton && qilNode2.NodeType == QilNodeType.LiteralInt32 && (int)((QilLiteral)qilNode2).Value == 1 && AllowReplace(XmlILOptimization.NormalizeMuenchian, local0))
				{
					QilNode qilNode27 = VisitFor(f.For(qilNode25));
					return Replace(XmlILOptimization.NormalizeMuenchian, local0, VisitIsEmpty(f.IsEmpty(VisitFilter(f.Filter(qilNode27, VisitNot(f.Not(VisitIs(f.Is(qilNode27, qilNode26)))))))));
				}
			}
		}
	...
	protected override QilNode VisitLoop(QilLoop local0)
	{
		...
		if (this[XmlILOptimization.NormalizeLoopConditional] && qilNode2.NodeType == QilNodeType.Conditional)
		{
			...
			if (qilNode5.NodeType == QilNodeType.Sequence && qilNode5.Count == 0 && qilNode6 == qilNode && AllowReplace(XmlILOptimization.NormalizeLoopConditional, local0))
			{
		...
		if (this[XmlILOptimization.NormalizeLoopConditional] && qilNode2.NodeType == QilNodeType.Conditional)
		{
			...
			if (qilNode7 == qilNode && qilNode8.NodeType == QilNodeType.Sequence && qilNode8.Count == 0 && AllowReplace(XmlILOptimization.NormalizeLoopConditional, local0))
			{
		...
		if (this[XmlILOptimization.NormalizeLoopConditional] && qilNode.NodeType == QilNodeType.For && qilNode2.NodeType == QilNodeType.Conditional)
		{
			...
			if (qilNode9.NodeType == QilNodeType.Sequence && qilNode9.Count == 0 && NonPositional(expr, qilNode) && AllowReplace(XmlILOptimization.NormalizeLoopConditional, local0))
			{
				QilNode qilNode10 = VisitFor(f.For(VisitFilter(f.Filter(qilNode, VisitNot(f.Not(child2))))));
				return Replace(XmlILOptimization.NormalizeLoopConditional, local0, VisitLoop(f.Loop(qilNode10, Subs(expr, qilNode, qilNode10))));
			}
		}
		if (this[XmlILOptimization.NormalizeLoopConditional] && qilNode.NodeType == QilNodeType.For && qilNode2.NodeType == QilNodeType.Conditional)
		{
			...
			if (NonPositional(expr2, qilNode) && qilNode11.NodeType == QilNodeType.Sequence && qilNode11.Count == 0 && AllowReplace(XmlILOptimization.NormalizeLoopConditional, local0))
			{
				QilNode qilNode12 = VisitFor(f.For(VisitFilter(f.Filter(qilNode, body3))));
				return Replace(XmlILOptimization.NormalizeLoopConditional, local0, VisitLoop(f.Loop(qilNode12, Subs(expr2, qilNode, qilNode12))));
			}
		}
		if (this[XmlILOptimization.NormalizeLoopLoop] && qilNode2.NodeType == QilNodeType.Loop)
		{
			...
			if (qilNode13.NodeType == QilNodeType.For)
			{
				...
				if (!DependsOn(expr3, qilNode) && NonPositional(expr3, qilNode13) && AllowReplace(XmlILOptimization.NormalizeLoopLoop, local0))
				{
					QilNode qilNode14 = VisitFor(f.For(VisitLoop(f.Loop(qilNode, body4))));
					return Replace(XmlILOptimization.NormalizeLoopLoop, local0, VisitLoop(f.Loop(qilNode14, Subs(expr3, qilNode13, qilNode14))));
				}
			}
		}
		if (this[XmlILOptimization.AnnotateSingletonLoop] && qilNode.NodeType == QilNodeType.For && !qilNode[0].XmlType.MaybeMany && AllowReplace(XmlILOptimization.AnnotateSingletonLoop, local0))
		{
		...
		if (this[XmlILOptimization.AnnotateDescendantLoop] && qilNode.NodeType == QilNodeType.For)
		{
			...
			if (OptimizerPatterns.Read(qilNode16).MatchesPattern(OptimizerPatternName.SameDepth) && (IsStepPattern(qilNode2, QilNodeType.Descendant) || IsStepPattern(qilNode2, QilNodeType.DescendantOrSelf)) && qilNode == OptimizerPatterns.Read(qilNode2).GetArgument(OptimizerPatternArgument.StepInput) && AllowReplace(XmlILOptimization.AnnotateDescendantLoop, local0))
			{
	...
	protected override QilNode VisitFilter(QilLoop local0)
	{
				...
				if (qilNode2.NodeType == QilNodeType.And)
				{
					...
					if (qilNode4.NodeType == QilNodeType.IsType)
					{
						...
						if (qilNode6 == qilNode && qilNode7.NodeType == QilNodeType.LiteralType && (XmlQueryType)((QilLiteral)qilNode7).Value == XmlQueryTypeFactory.Attribute && qilNode5.NodeType == QilNodeType.Eq)
						{
							...
							if (qilNode8.NodeType == QilNodeType.NameOf && qilNode8[0] == qilNode && qilNode9.NodeType == QilNodeType.LiteralQName && AllowReplace(XmlILOptimization.NormalizeAttribute, local0))
							{
		...
		if (this[XmlILOptimization.CommuteFilterLoop] && qilNode.NodeType == QilNodeType.For)
		{
			...
			if (qilNode10.NodeType == QilNodeType.Loop)
			{
				...
				if (NonPositional(qilNode2, qilNode) && !IsDocOrderDistinct(qilNode10) && AllowReplace(XmlILOptimization.CommuteFilterLoop, local0))
				{
					QilNode qilNode11 = VisitFor(f.For(binding));
					return Replace(XmlILOptimization.CommuteFilterLoop, local0, VisitLoop(f.Loop(variable, VisitFilter(f.Filter(qilNode11, Subs(qilNode2, qilNode, qilNode11))))));
				}
			}
		}
		...
		if (this[XmlILOptimization.AnnotateFilterElements] && qilNode.NodeType == QilNodeType.For && OptimizerPatterns.Read(qilNode[0]).MatchesPattern(OptimizerPatternName.Axis) && qilNode2.NodeType == QilNodeType.And)
		{
			...
			if (qilNode18.NodeType == QilNodeType.IsType)
			{
				...
				if (qilNode20 == qilNode && qilNode21.NodeType == QilNodeType.LiteralType && (XmlQueryType)((QilLiteral)qilNode21).Value == XmlQueryTypeFactory.Element && qilNode19.NodeType == QilNodeType.Eq)
				{
					...
					if (qilNode22.NodeType == QilNodeType.NameOf && qilNode22[0] == qilNode && qilNode23.NodeType == QilNodeType.LiteralQName && AllowReplace(XmlILOptimization.AnnotateFilterElements, local0))
					{
		...
		if (this[XmlILOptimization.AnnotateFilterContentKind] && qilNode.NodeType == QilNodeType.For && OptimizerPatterns.Read(qilNode[0]).MatchesPattern(OptimizerPatternName.Axis) && qilNode2.NodeType == QilNodeType.IsType)
		{
			...
			if (qilNode24 == qilNode && qilNode25.NodeType == QilNodeType.LiteralType)
			{
				...
				if (MatchesContentTest(xmlQueryType) && AllowReplace(XmlILOptimization.AnnotateFilterContentKind, local0))
				{
		...
		if (this[XmlILOptimization.AnnotateFilterAttributeKind] && qilNode.NodeType == QilNodeType.For && qilNode[0].NodeType == QilNodeType.Content && qilNode2.NodeType == QilNodeType.IsType)
		{
			...
			if (qilNode26 == qilNode && qilNode27.NodeType == QilNodeType.LiteralType && (XmlQueryType)((QilLiteral)qilNode27).Value == XmlQueryTypeFactory.Attribute && AllowReplace(XmlILOptimization.AnnotateFilterAttributeKind, local0))
			{
