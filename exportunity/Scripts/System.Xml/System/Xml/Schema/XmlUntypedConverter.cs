...
internal class XmlUntypedConverter : XmlListConverter
{
	...
	protected override object ChangeListType(object value, Type destinationType, IXmlNamespaceResolver nsResolver)
	{
		...
		if (atomicConverter == null || (!allowListToList && type != XmlBaseConverter.StringType && destinationType != XmlBaseConverter.StringType))
		{
			...
			{
				throw new InvalidCastException(Res.GetString("Xml type '{0}' cannot convert from Clr type '{1}' unless the destination type is String or XmlAtomicValue.", base.XmlTypeName, type.Name));
			}
			if (SupportsType(destinationType))
			{
				throw new InvalidCastException(Res.GetString("Xml type '{0}' cannot convert to Clr type '{1}' unless the source value is a String or an XmlAtomicValue.", base.XmlTypeName, destinationType.Name));
			}
