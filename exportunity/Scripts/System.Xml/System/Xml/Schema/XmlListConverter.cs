...
internal class XmlListConverter : XmlBaseConverter
{
	...
	private new Exception CreateInvalidClrMappingException(Type sourceType, Type destinationType)
	{
		...
		{
			return new InvalidCastException(Res.GetString("Xml type 'List of {0}' does not support Clr type '{1}'.", base.XmlTypeName, sourceType.Name));
		}
		return new InvalidCastException(Res.GetString("Xml type 'List of {0}' does not support a conversion from Clr type '{1}' to Clr type '{2}'.", base.XmlTypeName, sourceType.Name, destinationType.Name));
	}
}
