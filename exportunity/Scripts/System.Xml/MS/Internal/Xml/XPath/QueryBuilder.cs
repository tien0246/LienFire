...
internal sealed class QueryBuilder
{
	...
	private Query ProcessAxis(Axis root, Flags flags, out Props props)
	{
		...
		case Axis.AxisType.Namespace:
			...
			break;
		default:
			throw XPathException.Create("The XPath query '{0}' is not supported.", _query);
		}
		return query;
	}
	...
	private Query ProcessFilter(Filter root, Flags flags, out Props props)
	{
		...
		if (CanBeNumber(query) || (props2 & (Props)6) != Props.None)
		{
		...
		if ((props & Props.PosFilter) != Props.None && query2 is DocumentOrderQuery)
		{
		...
		if (num && _firstInput != null)
		{
			if (flag && (props & Props.PosFilter) != Props.None)
			{
	...
	private Query ProcessVariable(Variable root)
	{
		...
		if (!_allowVar)
		{
			throw XPathException.Create("'{0}' is an invalid key pattern. It either contains a variable reference or 'key()' function.", _query);
		}
	...
	private Query ProcessFunction(Function root, out Props props)
	{
		...
		{
			IDQuery result2 = new IDQuery(ProcessNode(root.ArgumentList[0], Flags.None, out props));
			props |= Props.NonFlat;
			return result2;
		}
