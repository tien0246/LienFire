...
internal class XPathParser
{
	...
	private AstNode ParseNodeTest(AstNode qyInput, Axis.AxisType axisType, XPathNodeType nodeType)
	{
		...
		case XPathScanner.LexKind.Star:
			...
			break;
		default:
			throw XPathException.Create("Expression must evaluate to a node-set.", _scanner.SourceText);
		}
	...
	private AstNode ParseMethod(AstNode qyInput)
	{
		...
		if (prefix.Length == 0 && s_functionTable.TryGetValue(name, out var value))
		{
			...
			{
				throw XPathException.Create("Function '{0}' in '{1}' has an invalid number of arguments.", name, _scanner.SourceText);
			}
			...
			else
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					case XPathResultType.NodeSet:
						...
						{
							throw XPathException.Create("The argument to function '{0}' in '{1}' cannot be converted to a node-set.", name, _scanner.SourceText);
						}
						break;
	...
	private AstNode ParseStepPattern(AstNode qyInput)
	{
		...
		case XPathScanner.LexKind.Axe:
			...
			{
				throw XPathException.Create("'{0}' has an invalid token.", _scanner.SourceText);
			}
			NextLex();
			break;
		}
	...
	{
		return new Dictionary<string, ParamInfo>(36)
		{
			{
				"last",
				new ParamInfo(Function.FunctionType.FuncLast, 0, 0, s_temparray1)
			},
			{
				"position",
			...
			{
				"name",
				new ParamInfo(Function.FunctionType.FuncName, 0, 1, s_temparray2)
			},
			{
				"namespace-uri",
			...
			{
				"count",
				new ParamInfo(Function.FunctionType.FuncCount, 1, 1, s_temparray2)
			},
			{
				"id",
				new ParamInfo(Function.FunctionType.FuncID, 1, 1, s_temparray3)
			},
			{
				"string",
			...
			{
				"not",
				new ParamInfo(Function.FunctionType.FuncNot, 1, 1, s_temparray8)
			},
			{
				"true",
				new ParamInfo(Function.FunctionType.FuncTrue, 0, 0, s_temparray8)
			},
			{
				"false",
				new ParamInfo(Function.FunctionType.FuncFalse, 0, 0, s_temparray8)
			},
			{
				"lang",
				new ParamInfo(Function.FunctionType.FuncLang, 1, 1, s_temparray4)
			},
			{
				"number",
			...
			{
				"sum",
				new ParamInfo(Function.FunctionType.FuncSum, 1, 1, s_temparray2)
			},
			{
				"floor",
				new ParamInfo(Function.FunctionType.FuncFloor, 1, 1, s_temparray9)
			},
			{
				"ceiling",
			...
			{
				"round",
				new ParamInfo(Function.FunctionType.FuncRound, 1, 1, s_temparray9)
			}
		};
	}
