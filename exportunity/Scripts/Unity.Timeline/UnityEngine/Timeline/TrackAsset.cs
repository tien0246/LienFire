...
public abstract class TrackAsset : PlayableAsset, ISerializationCallbackReceiver, IPropertyPreview, ICurvesOwner
{
	...
	private Playable CreateNotificationsPlayable(PlayableGraph graph, Playable mixerPlayable, GameObject go, Playable timelinePlayable)
	{
		...
		if (go.TryGetComponent<PlayableDirector>(out var component))
		{
			extrapolationMode = component.extrapolationMode;
		}
		double num = timelineAsset.duration;
		ScriptPlayable<TimeNotificationBehaviour> scriptPlayable = NotificationUtilities.CreateNotificationsPlayable(graph, s_BuildData.markerList, num, extrapolationMode);
