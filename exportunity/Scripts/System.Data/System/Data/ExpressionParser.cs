...
internal sealed class ExpressionParser
{
		...
		while (_token != Tokens.EOS)
		{
			while (true)
			{
				...
				case Tokens.LeftParen:
				{
					...
					node = new FunctionNode(_table, nameNode._name);
				...
				case Tokens.Dot:
				{
					...
					if (expressionNode != null && expressionNode.GetType() == typeof(NameNode))
					{
						...
						{
							string name = ((NameNode)NodePop())._name + "." + NameNode.ParseName(_text, _start, _pos);
	...
	private void ScanName(char chEnd, char esc, string charsToEscape)
	{
		...
		do
		{
			if (text[_pos] == esc && _pos + 1 < text.Length && charsToEscape.IndexOf(text[_pos + 1]) >= 0)
			{
				_pos++;
			}
			_pos++;
