...
internal sealed class FunctionNode : ExpressionNode
{
	...
	internal bool IsAggregate
	{
		get
		{
			if (s_funcs[_info]._id != FunctionId.Sum && s_funcs[_info]._id != FunctionId.Avg && s_funcs[_info]._id != FunctionId.Min && s_funcs[_info]._id != FunctionId.Max && s_funcs[_info]._id != FunctionId.Count && s_funcs[_info]._id != FunctionId.StDev)
			{
				return s_funcs[_info]._id == FunctionId.Var;
			}
			return true;
		}
	}
	...
	internal void AddArgument(ExpressionNode argument)
	{
		...
		else if (_argumentCount == _arguments.Length)
		{
			...
			Array.Copy(_arguments, 0, array, 0, _argumentCount);
			_arguments = array;
		}
	...
	private object EvalFunction(FunctionId id, object[] argumentValues, DataRow row, DataRowVersion version)
	{
		...
		case FunctionId.Iif:
			if (DataExpression.ToBoolean(_arguments[0].Eval(row, version)))
			{
