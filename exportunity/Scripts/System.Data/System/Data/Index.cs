...
internal sealed class Index
{
	...
	{
		if (_table._recordManager[record1] == null)
		{
			if (_table._recordManager[record2] != null)
			{
				return -1;
			}
			return 0;
		...
		int num = _table._recordManager[record1].rowID.CompareTo(_table._recordManager[record2].rowID);
		if (num == 0 && record1 != record2)
		{
			num = ((int)_table._recordManager[record1].GetRecordState(record1)).CompareTo((int)_table._recordManager[record2].GetRecordState(record2));
		}
		return num;
	}
	...
	private int FindNodeByKeys(object[] originalKey)
	{
		...
		if (num == 0 || _indexFields.Length != num)
		{
	...
	{
		int num = (((_recordStates & oldState) != DataViewRowState.None) ? 1 : 0);
		return (((_recordStates & newState) != DataViewRowState.None) ? 1 : 0) - num;
	}
