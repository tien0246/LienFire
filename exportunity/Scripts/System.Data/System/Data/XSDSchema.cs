...
internal sealed class XSDSchema : XMLSchema
{
	...
	private void CollectElementsAnnotations(XmlSchema schema, ArrayList schemaList)
	{
		...
		foreach (XmlSchemaObject item in schema.Items)
		{
			...
			{
				GetMsdataAttribute((XmlSchemaType)item, "targetNamespace");
			}
	...
	internal DataTable InstantiateSimpleTable(XmlSchemaElement node)
	{
		...
		DataTable dataTable = _ds.Tables.GetTable(text, text2);
		...
		if (dataTable == null)
		{
			...
			dataTable.Namespace = text2;
			dataTable.Namespace = GetStringAttribute(node, "targetNamespace", text2);
			if (!FromInference)
			{
		...
		if (!FromInference || (flag && dataTable.Columns.Count == 0))
		{
			...
			if (FromInference)
			{
				int num = 0;
				text3 = text + "_Text";
				while (dataTable.Columns[text3] != null)
				{
					text3 += num++;
				}
			}
			...
			dataTable.Columns[0].ColumnName = text3;
			dataTable.Columns[0].ColumnMapping = MappingType.SimpleContent;
		}
		if (!FromInference || _ds.Tables.GetTable(text, text2) == null)
		{
	...
	internal DataTable InstantiateTable(XmlSchemaElement node, XmlSchemaComplexType typeNode, bool isRef)
	{
		...
		ArrayList arrayList = new ArrayList();
		string text = node.QualifiedName.Namespace;
		DataTable dataTable = _ds.Tables.GetTable(XmlConvert.DecodeName(instanceName), text);
	...
	private Type ParseDataType(string dt)
	{
		...
		{
			SimpleType simpleType = new SimpleType(((XmlSchemaSimpleType)_udSimpleTypes[dt]) ?? throw ExceptionBuilder.UndefinedDatatype(dt));
	...
	{
		if (Array.BinarySearch(s_mapNameTypeXsd, name) < 0)
		{
			return false;
		}
		return true;
	...
	internal void HandleElementColumn(XmlSchemaElement elem, DataTable table, bool isBase)
	{
		...
		else if (xmlSchemaAnnotated is XmlSchemaSimpleType)
		{
			...
			{
				GetMsdataAttribute(xmlSchemaAnnotated, "targetNamespace");
		...
		int num = (int)Convert.ChangeType(GetStringAttribute(elem, "Ordinal", (-1).ToString(CultureInfo.CurrentCulture)), typeof(int), null);
		if (flag)
		{
