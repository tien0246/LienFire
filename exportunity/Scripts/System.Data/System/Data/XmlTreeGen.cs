...
internal sealed class XmlTreeGen
{
	...
	internal void AddXdoProperty(PropertyDescriptor pd, object instance, XmlElement root, XmlDocument xd)
	{
		...
		if ((!flag3 && propertyType != typeof(string) && propertyType != typeof(bool) && propertyType != typeof(Type) && propertyType != typeof(object) && propertyType != typeof(CultureInfo) && propertyType != typeof(long) && propertyType != typeof(int)) || ((!pd.ShouldSerializeValue(instance) || !pd.Attributes.Contains(DesignerSerializationVisibilityAttribute.Visible)) && !flag2))
		{
			return;
		}
		...
		if (col.SimpleType != null)
		{
			for (SimpleType simpleType = col.SimpleType; simpleType != null; simpleType = simpleType.BaseSimpleType)
			{
				...
				if (name2 != null && name2.Length != 0)
				{
					...
					if (FindSimpleType(schema2, name2) == null)
					{
	...
	internal XmlElement HandleColumn(DataColumn col, XmlDocument dc, XmlElement schema, bool fWriteOrdinal)
	{
		...
		else if (col.Namespace != (col.Table.TypeName.IsEmpty ? col.Table.Namespace : col.Table.TypeName.Namespace) && col.Namespace.Length != 0)
		{
			...
			{
				_ = (string)_prefixes[col.Namespace];
	...
	internal XmlElement HandleTable(DataTable table, XmlDocument dc, XmlElement schema, bool genNested)
	{
		...
		{
			string text2 = XmlDataTypeName(table.Columns[0].DataType);
			if (text2 == null || text2.Length == 0)
			{
				text2 = "anyType";
			}
		...
		for (int l = 0; l < count; l++)
		{
			...
			if (dataColumn2.ColumnMapping != MappingType.SimpleContent && (dataColumn2.ColumnMapping == MappingType.Attribute || dataColumn2.ColumnMapping == MappingType.Element || dataColumn2.ColumnMapping == MappingType.Hidden) && !IsAutoGenerated(dataColumn2))
			{
				bool num3 = dataColumn2.ColumnMapping != MappingType.Element;
				XmlElement newChild = HandleColumn(dataColumn2, dc, schema, flag);
				(num3 ? xmlElement2 : xmlElement4).AppendChild(newChild);
			}
		}
		if (table.XmlText == null && genNested)
		{
			...
			for (int m = 0; m < childRelations2.Count; m++)
			{
				...
				{
					xmlElement7 = HandleTable(childTable, dc, schema);
				}
				...
				else
				{
					...
					{
						GetSchema(childTable.Namespace).AppendChild(xmlElement7);
					}
					xmlElement7 = dc.CreateElement("xs", "element", "http://www.w3.org/2001/XMLSchema");
					xmlElement7.SetAttribute("ref", (string)_prefixes[childTable.Namespace] + ":" + childTable.EncodedTableName);
					xmlElement4.AppendChild(xmlElement7);
				}
				if (childRelations2[m].ChildKeyConstraint == null)
				{
					XmlElement xmlElement8 = _dc.CreateElement("xs", "annotation", "http://www.w3.org/2001/XMLSchema");
					xmlElement7.PrependChild(xmlElement8);
					XmlElement xmlElement9 = _dc.CreateElement("xs", "appinfo", "http://www.w3.org/2001/XMLSchema");
					xmlElement8.AppendChild(xmlElement9);
					xmlElement9.AppendChild(HandleRelation(childRelations2[m], dc));
				}
			}
		}
		...
		for (int n = 0; n < constraints.Count; n++)
		{
			...
			if (constraints[n] is UniqueConstraint)
			{
				...
				xmlElement10 = dc.CreateElement("xs", "unique", "http://www.w3.org/2001/XMLSchema");
				...
				AddExtendedProperties(uniqueConstraint._extendedProperties, xmlElement10);
				...
				if (columnsReference.Length != 0)
				{
					...
					for (int num4 = 0; num4 < columnsReference.Length; num4++)
					{
						...
						if (_schFormat != SchemaFormat.Remoting)
						{
							...
							{
								stringBuilder.Append(_prefixes[columnsReference[num4].Namespace]).Append(':');
							}
							stringBuilder.Append(columnsReference[num4].EncodedColumnName);
						}
						else
						{
							stringBuilder.Append(text3).Append(columnsReference[num4].EncodedColumnName);
						}
				...
				_dsElement.InsertBefore(xmlElement10, _constraintSeparator);
			}
			else
			{
					...
					if (columnsReference.Length != 0)
					{
						...
						for (int num5 = 0; num5 < columnsReference.Length; num5++)
						{
							...
							if (_schFormat != SchemaFormat.Remoting)
							{
								...
								{
									stringBuilder2.Append(_prefixes[columnsReference[num5].Namespace]).Append(':');
								}
								stringBuilder2.Append(columnsReference[num5].EncodedColumnName);
							}
							else
							{
								stringBuilder2.Append(text3).Append(columnsReference[num5].EncodedColumnName);
							}
				...
				AddExtendedProperties(foreignKeyConstraint._extendedProperties, xmlElement10, typeof(ForeignKeyConstraint));
				...
				else
				{
					...
					AddExtendedProperties(dataRelation._extendedProperties, xmlElement10, typeof(DataRelation));
				...
				{
					xmlElement10.SetAttribute("AcceptRejectRule", "urn:schemas-microsoft-com:xml-msdata", TranslateAcceptRejectRule(foreignKeyConstraint.AcceptRejectRule));
				}
				if (foreignKeyConstraint.UpdateRule != Rule.Cascade)
				{
					xmlElement10.SetAttribute("UpdateRule", "urn:schemas-microsoft-com:xml-msdata", TranslateRule(foreignKeyConstraint.UpdateRule));
				}
				if (foreignKeyConstraint.DeleteRule != Rule.Cascade)
				{
					xmlElement10.SetAttribute("DeleteRule", "urn:schemas-microsoft-com:xml-msdata", TranslateRule(foreignKeyConstraint.DeleteRule));
				}
				...
				if (columnsReference.Length != 0)
				{
					...
					for (int num6 = 0; num6 < columnsReference.Length; num6++)
					{
						...
						if (_schFormat != SchemaFormat.Remoting)
						{
							GetSchema(columnsReference[num6].Namespace);
							if (!string.IsNullOrEmpty(columnsReference[num6].Namespace))
							{
								stringBuilder3.Append(_prefixes[columnsReference[num6].Namespace]).Append(':');
							}
							stringBuilder3.Append(columnsReference[num6].EncodedColumnName);
						}
						else
						{
							stringBuilder3.Append(text3).Append(columnsReference[num6].EncodedColumnName);
						}
						if (columnsReference[num6].ColumnMapping == MappingType.Attribute || columnsReference[num6].ColumnMapping == MappingType.Hidden)
						{
				...
				_dsElement.InsertAfter(xmlElement10, _constraintSeparator);
			}
		}
