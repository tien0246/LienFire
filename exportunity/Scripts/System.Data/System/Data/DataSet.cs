...
public class DataSet : MarshalByValueComponent, IListSource, IXmlSerializable, ISupportInitializeNotification, ISupportInitialize, ISerializable
{
	...
	private void SerializeDataSet(SerializationInfo info, StreamingContext context, SerializationFormat remotingFormat)
	{
		...
		else
		{
			...
			StringWriter stringWriter = new StringWriter(new StringBuilder(EstimatedXmlStringSize() * 2), CultureInfo.InvariantCulture);
	...
	{
		foreach (ArrayList item in (ArrayList)info.GetValue("DataSet.Relations", typeof(ArrayList)))
		{
		...
		try
		{
			...
			foreach (DataTable table in Tables)
			{
				foreach (DataColumn column in table.Columns)
				{
					...
					{
						dataSet.Tables[table.TableName, table.Namespace].Columns[column.ColumnName].Expression = column.Expression;
					}
				}
			}
	...
	public DataSet GetChanges(DataRowState rowStates)
	{
		...
		try
		{
			...
			if ((rowStates & ~(DataRowState.Unchanged | DataRowState.Added | DataRowState.Deleted | DataRowState.Modified)) != 0)
			{
	...
	internal string GetRemotingDiffGram(DataTable table)
	{
		...
		{
			new NewDiffgramGen(table, writeHierarchy: false).Save(xmlw, table);
		}
	...
	public string GetXmlSchema()
	{
		...
		{
			StringWriter stringWriter = new StringWriter(CultureInfo.InvariantCulture);
			XmlTextWriter xw = new XmlTextWriter(stringWriter)
			{
				Formatting = Formatting.Indented
			};
			if (stringWriter != null)
	...
	public bool HasChanges(DataRowState rowStates)
	{
		...
		try
		{
			if ((rowStates & ~(DataRowState.Detached | DataRowState.Unchanged | DataRowState.Added | DataRowState.Deleted | DataRowState.Modified)) != 0)
			{
	...
	internal void InferSchema(XmlDocument xdoc, string[] excludedNamespaces, XmlReadMode mode)
	{
		...
		try
		{
			_ = xdoc.DocumentElement.NamespaceURI;
			...
			XmlSchemaSet xmlSchemaSet = new XmlSchemaInference
			{
				...
				TypeInference = ((mode != XmlReadMode.InferTypedSchema) ? XmlSchemaInference.InferenceOption.Relaxed : XmlSchemaInference.InferenceOption.Restricted)
	...
	public XmlReadMode ReadXml(string fileName, XmlReadMode mode)
	{
		...
		xmlTextReader = ((mode != XmlReadMode.Fragment) ? new XmlTextReader(fileName) : new XmlTextReader(new FileStream(fileName, FileMode.Open), XmlNodeType.Element, null));
		xmlTextReader.XmlResolver = null;
		try
		{
			return ReadXml(xmlTextReader, mode, denyResolving: false);
