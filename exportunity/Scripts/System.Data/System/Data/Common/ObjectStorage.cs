...
internal sealed class ObjectStorage : DataStorage
{
	...
	internal ObjectStorage(DataColumn column, Type type)
		: base(column, type, s_defaultValue, DBNull.Value, typeof(ICloneable).IsAssignableFrom(type), DataStorage.GetStorageType(type))
	{
	...
	public override object ConvertXmlToObject(XmlReader xmlReader, XmlRootAttribute xmlAttrib)
	{
		...
		{
			obj = GetXmlSerializer(_dataType, xmlAttrib).Deserialize(xmlReader);
		}
		return obj;
	}
	...
	public override string ConvertObjectToXml(object value)
	{
		...
		GetXmlSerializer(value.GetType()).Serialize(stringWriter, value);
		return stringWriter.ToString();
	}
	...
	protected override void SetStorage(object store, BitArray nullbits)
	{
		...
		for (int i = 0; i < _values.Length; i++)
		{
			...
			{
				DateTime value = (DateTime)_values[i];
