...
internal sealed class Merger
{
	...
	internal void MergeDataSet(DataSet source)
	{
		...
		if (MissingSchemaAction.Add == _missingSchemaAction)
		{
			...
			{
				DataTable dataTable2 = ((!_IgnoreNSforTableLookup) ? _dataSet.Tables[table2.TableName, table2.Namespace] : _dataSet.Tables[table2.TableName]);
				foreach (DataColumn column2 in table2.Columns)
				{
					if (column2.Computed)
					{
						DataColumn dataColumn2 = dataTable2.Columns[column2.ColumnName];
						if (!list.Contains(dataColumn2))
						{
							dataColumn2.Expression = column2.Expression;
						}
					}
				}
			}
			...
			if (MissingSchemaAction.Ignore != _missingSchemaAction)
			{
				...
				for (int i = 0; i < table.Columns.Count; i++)
				{
					...
					if (dataColumn2 == null)
					{
						...
						if (!_isStandAlonetable)
						{
							_dataSet.RaiseMergeFailed(dataTable, global::SR.Format("Target table {0} missing definition for column {1}.", table.TableName, dataColumn.ColumnName), _missingSchemaAction);
							continue;
						}
						throw ExceptionBuilder.MergeFailed(global::SR.Format("Target table {0} missing definition for column {1}.", table.TableName, dataColumn.ColumnName));
					}
					if (dataColumn2.DataType != dataColumn.DataType || (dataColumn2.DataType == typeof(DateTime) && dataColumn2.DateTimeMode != dataColumn.DateTimeMode && (dataColumn2.DateTimeMode & dataColumn.DateTimeMode) != DataSetDateTime.Unspecified))
					{
						if (_isStandAlonetable)
						{
							throw ExceptionBuilder.MergeFailed(global::SR.Format("<target>.{0} and <source>.{0} have conflicting properties: DataType property mismatch.", dataColumn.ColumnName));
						}
						_dataSet.RaiseMergeFailed(dataTable, global::SR.Format("<target>.{0} and <source>.{0} have conflicting properties: DataType property mismatch.", dataColumn.ColumnName), MissingSchemaAction.Error);
					}
				...
				if (primaryKey.Length != primaryKey2.Length)
				{
					...
					{
						_dataSet.RaiseMergeFailed(dataTable, "<target>.PrimaryKey and <source>.PrimaryKey have different Length.", _missingSchemaAction);
					}
				}
				else
				{
					for (int l = 0; l < primaryKey.Length; l++)
					{
						...
						{
							_dataSet.RaiseMergeFailed(table, global::SR.Format("Mismatch columns in the PrimaryKey : <target>.{0} versus <source>.{1}.", primaryKey[l].ColumnName, primaryKey2[l].ColumnName), _missingSchemaAction);
						}
					}
				}
			}
	...
	private void MergeConstraints(DataTable table)
	{
		for (int i = 0; i < table.Constraints.Count; i++)
		{
			...
			if (constraint2 == null)
			{
				_dataSet.RaiseMergeFailed(table, global::SR.Format("Target DataSet missing {0} {1}.", constraint.GetType().FullName, constraint.ConstraintName), _missingSchemaAction);
				continue;
			}
	...
	private void MergeRelation(DataRelation relation)
	{
		...
		if (num >= 0)
		{
			...
			{
				_dataSet.RaiseMergeFailed(null, global::SR.Format("Target DataSet missing definition for {0}.", relation.RelationName), _missingSchemaAction);
			}
			for (int i = 0; i < relation.ParentKey.ColumnsReference.Length; i++)
			{
				...
				{
					_dataSet.RaiseMergeFailed(null, global::SR.Format("Relation {0} cannot be merged, because keys have mismatch columns.", relation.RelationName), _missingSchemaAction);
				}
