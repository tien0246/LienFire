...
internal sealed class Select
{
	...
	{
		IndexField[] indexFields = _index._indexFields;
		_ = indexFields.Length;
		for (int i = 0; i < _matchedCandidates; i++)
		{
			_candidateColumns[indexFields[i].Column.Ordinal].flag = true;
		}
		int num = _candidateColumns.Length;
		for (int i = 0; i < num; i++)
		{
			...
			if (!_candidateColumns[i].flag)
			{
				...
				{
					_linearExpression = ((_linearExpression == null) ? _candidateColumns[i].expr : new BinaryNode(_table, 26, _candidateColumns[i].expr, _linearExpression));
				}
			}
			else
			{
	...
	private int Evaluate(int record)
	{
		...
		for (int i = 0; i < _matchedCandidates; i++)
		{
			...
			int num = Eval(_candidateColumns[ordinal].expr, dataRow, version);
			if (num != 0)
			{
	...
	private int CompareRecords(int record1, int record2)
	{
		...
		long num3 = ((_table._recordManager[record1] == null) ? 0 : _table._recordManager[record1].rowID);
		long num4 = ((_table._recordManager[record2] == null) ? 0 : _table._recordManager[record2].rowID);
		int num5 = ((num3 < num4) ? (-1) : ((num4 < num3) ? 1 : 0));
		if (num5 == 0 && record1 != record2 && _table._recordManager[record1] != null && _table._recordManager[record2] != null)
		{
			num3 = (long)_table._recordManager[record1].GetRecordState(record1);
			num4 = (long)_table._recordManager[record2].GetRecordState(record2);
