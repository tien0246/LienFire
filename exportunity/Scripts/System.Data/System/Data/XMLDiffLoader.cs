	...
	private int ReadOldRowData(DataSet ds, ref DataTable table, ref int pos, XmlReader row)
	{
		...
		else
		{
			while (row.Depth > depth)
			{
				...
				{
					bool num2 = dataColumn2.DataType == typeof(object) || row.GetAttribute("InstanceType", "urn:schemas-microsoft-com:xml-msdata") != null || row.GetAttribute("type", "http://www.w3.org/2001/XMLSchema-instance") != null;
					bool flag = false;
					...
					if (!num2 && !dataColumn2.ImplementsIXMLSerializable)
					{
						if (flag)
						{
