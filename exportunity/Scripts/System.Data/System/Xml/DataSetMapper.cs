...
internal sealed class DataSetMapper
{
	...
	internal DataTable SearchMatchingTableSchema(XmlBoundElement rowElem, XmlBoundElement elem)
	{
		...
		if (GetColumnSchemaForNode(rowElem, elem) == null)
		{
			return dataTable;
		}
	...
	internal bool IsRegionRadical(XmlBoundElement rowElem)
	{
		...
		DataColumnCollection columns = GetTableSchemaForElement(rowElem).Columns;
		int iColumn = 0;
		...
		while (xmlNode != null)
		{
			...
			if (GetRowFromElement((XmlElement)xmlNode) == null)
			{
				return false;
			}
