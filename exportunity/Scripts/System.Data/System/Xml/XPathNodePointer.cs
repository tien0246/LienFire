...
internal sealed class XPathNodePointer : IXmlDataVirtualNode
{
	...
	static XPathNodePointer()
	{
		...
		s_xmlNodeType_To_XpathNodeType_Map[0] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[1] = 1;
		s_xmlNodeType_To_XpathNodeType_Map[2] = 2;
		s_xmlNodeType_To_XpathNodeType_Map[3] = 4;
		s_xmlNodeType_To_XpathNodeType_Map[4] = 4;
		s_xmlNodeType_To_XpathNodeType_Map[5] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[6] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[7] = 7;
		s_xmlNodeType_To_XpathNodeType_Map[8] = 8;
		s_xmlNodeType_To_XpathNodeType_Map[9] = 0;
		s_xmlNodeType_To_XpathNodeType_Map[10] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[11] = 0;
		s_xmlNodeType_To_XpathNodeType_Map[12] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[13] = 6;
		s_xmlNodeType_To_XpathNodeType_Map[14] = 5;
		s_xmlNodeType_To_XpathNodeType_Map[15] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[16] = -1;
		s_xmlNodeType_To_XpathNodeType_Map[17] = -1;
	}
		...
		if (_node != null)
		{
			...
			if (bFirst)
			{
				foreach (XmlAttribute attribute in _node.Attributes)
				{
					if (attribute.NamespaceURI != "http://www.w3.org/2000/xmlns/")
					{
						MoveTo(attribute, null, fOnValue: false);
						return true;
					}
				}
			}
			...
			{
				XmlAttributeCollection attributes = ((XmlAttribute)_node).OwnerElement.Attributes;
				bool flag = false;
				foreach (XmlAttribute item in attributes)
				{
					if (flag && item.NamespaceURI != "http://www.w3.org/2000/xmlns/")
					{
	...
	private bool IsValidChild(XmlNode parent, XmlNode child)
	{
		...
		switch (s_xmlNodeType_To_XpathNodeType_Map[(int)parent.NodeType])
		{
		case 0:
			if (num != 1 && num != 8)
			{
			...
			return true;
		case 1:
			if (num != 1 && num != 4 && num != 8 && num != 6 && num != 5)
			{
				return num == 7;
			}
			return true;
	...
	private bool IsValidChild(XmlNode parent, DataColumn c)
	{
		...
		case 1:
			if (c.ColumnMapping != MappingType.Element)
			{
	...
	private void AssertValid()
	{
		if (_column != null)
		{
			_ = (_node as XmlBoundElement).Row.RowState;
			_ = 1;
		}
		_ = _column;
	}
