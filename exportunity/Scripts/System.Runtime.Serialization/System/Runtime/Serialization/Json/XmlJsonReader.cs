...
internal class XmlJsonReader : XmlBaseReader, IXmlJsonReaderInitializer
{
	...
	public override bool Read()
	{
		...
		else if (ch == 102)
		{
			...
			{
				string text = Encoding.UTF8.GetString(buffer, offset, 4);
				char c = (char)ch;
				XmlExceptionHelper.ThrowTokenExpected(this, "false", text + c);
			}
		...
		else if (ch == 116)
		{
			...
			if (TryGetByte(out ch) && !IsWhitespace(ch) && ch != 44 && ch != 125 && ch != 93)
			{
				string text2 = Encoding.UTF8.GetString(buffer2, offset2, 4);
				char c = (char)ch;
				XmlExceptionHelper.ThrowTokenExpected(this, "true", text2 + c);
			}
		...
		else if (ch == 110)
		{
			...
			if (TryGetByte(out ch))
			{
				...
				default:
				{
					string text3 = Encoding.UTF8.GetString(buffer3, offset3, 4);
					char c = (char)ch;
					XmlExceptionHelper.ThrowTokenExpected(this, "null", text3 + c);
					break;
				}
				case 93:
					break;
	...
	private void ParseAndSetLocalName()
	{
		...
		int num2 = base.BufferReader.GetByte(xmlElementNode.NameOffset);
		if ((charType[num2] & 1) == 0)
		{
			SetJsonNameWithMapping(xmlElementNode);
			return;
		}
		int num3 = 0;
		int num4 = xmlElementNode.NameOffset;
		while (num3 < xmlElementNode.NameLength)
		{
			num2 = base.BufferReader.GetByte(num4);
			if ((charType[num2] & 2) == 0 || num2 >= 128)
			{
				SetJsonNameWithMapping(xmlElementNode);
				break;
			}
	...
	private void ParseStartElement()
	{
		...
		{
			XmlExceptionHelper.ThrowTokenExpected(this, "\"", (char)b);
		}
	}
	...
	private void ReadAttributes()
	{
		...
		case 125:
			...
			XmlExceptionHelper.ThrowXmlException(this, new XmlException(SR.GetString("Encountered an unexpected character '{0}' in JSON.", (char)b)));
			return;
		case 91:
	...
	private void ReadEscapedCharacter(bool moveToText)
	{
		...
		case 'u':
		{
			...
			string text = Encoding.UTF8.GetString(buffer, offset, 4);
			base.BufferReader.Advance(4);
			int num = ParseChar(text, NumberStyles.HexNumber);
			if (char.IsHighSurrogate((char)num) && base.BufferReader.GetByte() == 92)
			{
				...
				text = Encoding.UTF8.GetString(buffer, offset, 4);
				base.BufferReader.Advance(4);
				char c2 = ParseChar(text, NumberStyles.HexNumber);
				if (!char.IsLowSurrogate(c2))
				{
					XmlExceptionHelper.ThrowXmlException(this, new XmlException(SR.GetString("Low surrogate char '0x{0}' not valid. Low surrogate chars range from 0xDC00 to 0xDFFF.", text)));
				}
	...
	private void SetJsonNameWithMapping(XmlElementNode elementNode)
	{
		...
		obj.Prefix.SetValue(PrefixHandleType.A);
		obj.Uri.SetConstantValue(StringHandleConstStringType.Item);
		AddXmlnsAttribute(obj);
		XmlAttributeNode xmlAttributeNode = AddAttribute();
