...
internal class XmlBinaryReader : XmlBaseReader, IXmlBinaryReaderInitializer
{
	...
	private void ReadAttributes2()
	{
		...
		while (true)
		{
			...
			case XmlBinaryNodeType.XmlnsAttribute:
			{
				...
				ReadName(obj.Prefix);
				ReadName(obj.Uri);
				XmlAttributeNode xmlAttributeNode = AddXmlnsAttribute(obj);
				break;
			}
			case XmlBinaryNodeType.ShortXmlnsAttribute:
			{
				SkipNodeType();
				Namespace obj = AddNamespace();
				obj.Prefix.SetValue(PrefixHandleType.Empty);
				ReadName(obj.Uri);
				XmlAttributeNode xmlAttributeNode = AddXmlnsAttribute(obj);
				break;
			}
	...
	private void ReadPartialUTF8Text(bool withEndElement, int length)
	{
		...
		{
			byte b = base.BufferReader.GetByte(num4);
			if ((b & 0x80) == 0 || (b & 0xC0) == 192)
			{
				break;
			}
		}
