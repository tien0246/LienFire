...
public sealed class TerrainData : Object
{
	...
	public int heightmapResolution
	{
		...
		set
		{
			...
			if (value < 0 || value > k_MaximumResolution)
			{
				...
				Debug.LogWarning("heightmapResolution is clamped to the range of [0, " + num2 + "].");
	...
	internal static int maxDetailsPerRes => k_MaximumDetailsPerRes;
	...
	public int alphamapResolution
	{
		...
		set
		{
			...
			{
				string[] obj = new string[5] { "alphamapResolution is clamped to the range of [", null, null, null, null };
				int num = k_MinimumAlphamapResolution;
				obj[1] = num.ToString();
				obj[2] = ", ";
				num = k_MaximumAlphamapResolution;
				obj[3] = num.ToString();
				obj[4] = "].";
	...
	public int baseMapResolution
	{
		...
		set
		{
			...
			{
				string[] obj = new string[5] { "baseMapResolution is clamped to the range of [", null, null, null, null };
				int num = k_MinimumBaseMapResolution;
				obj[1] = num.ToString();
				obj[2] = ", ";
				num = k_MaximumBaseMapResolution;
				obj[3] = num.ToString();
				obj[4] = "].";
				Debug.LogWarning(string.Concat(obj));
	...
	public void SetDetailResolution(int detailResolution, int resolutionPerPatch)
	{
		...
		{
			string[] obj = new string[5] { "resolutionPerPatch is clamped to the range of [", null, null, null, null };
			int num = k_MinimumDetailResolutionPerPatch;
			obj[1] = num.ToString();
			obj[2] = ", ";
			num = k_MaximumDetailResolutionPerPatch;
			obj[3] = num.ToString();
			obj[4] = "].";
			Debug.LogWarning(string.Concat(obj));
		...
		int num2 = detailResolution / resolutionPerPatch;
		if (num2 > k_MaximumDetailPatchCount)
		{
			int num = k_MaximumDetailPatchCount;
			Debug.LogWarning("Patch count (detailResolution / resolutionPerPatch) is clamped to the range of [0, " + num + "].");
			num2 = Math.Min(k_MaximumDetailPatchCount, Math.Max(num2, 0));
		}
		Internal_SetDetailResolution(num2, resolutionPerPatch);
	}
	...
	public void CopyActiveRenderTextureToTexture(string textureName, int textureIndex, RectInt sourceRect, Vector2Int dest, bool allowDelayedCPUSync)
	{
		...
		else
		{
			...
			{
				alphamapTexture.ReadPixels(new Rect(sourceRect.x, active.height - sourceRect.yMax, sourceRect.width, sourceRect.height), dest.x, dest.y);
			}
