...
internal class FAMWatcher : IFileWatcher
{
	...
	public void StartDispatching(object handle)
	{
		...
		lock (this)
		{
			...
			fAMData = (FAMData)watches[fileSystemWatcher];
		}
		if (fAMData == null)
		{
			fAMData = new FAMData();
			...
			lock (this)
			{
				watches[fileSystemWatcher] = fAMData;
	...
	private static void StartMonitoringDirectory(FAMData data, bool justcreated)
	{
		...
		{
			directories = Directory.GetDirectories(data.Directory);
		...
		directories = Directory.GetFiles(data.Directory);
	...
	public void StopDispatching(object handle)
	{
		...
		lock (this)
		{
			FAMData fAMData = (FAMData)watches[key];
			if (fAMData == null)
			{
				return;
			}
