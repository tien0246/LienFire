...
internal class MimeMultiPart : MimeBasePart
{
	...
	{
		((MimeWriter)((MimePartContext)result.AsyncState)._writer).EndClose(result);
	...
	private void MimePartSentCallbackHandler(IAsyncResult result)
	{
		...
		{
			IAsyncResult asyncResult = mimePartContext._partsEnumerator.Current.BeginSend(mimePartContext._writer, _mimePartSentCallback, _allowUnicode, mimePartContext);
	...
	{
		return "--boundary_" + (Interlocked.Increment(ref s_boundary) - 1).ToString(CultureInfo.InvariantCulture) + "_" + Guid.NewGuid().ToString(null, CultureInfo.InvariantCulture);
	}
}
