...
public class RayConverter : fsDirectConverter<Ray>
{
	...
	protected override fsResult DoDeserialize(Dictionary<string, fsData> data, ref Ray model)
	{
		...
		fsResult fsResult = success + DeserializeMember<Vector3>(data, null, "origin", out value);
		model.origin = value;
		Vector3 value2 = model.direction;
		fsResult result = fsResult + DeserializeMember<Vector3>(data, null, "direction", out value2);
