...
public class Ray2DConverter : fsDirectConverter<Ray2D>
{
	...
	protected override fsResult DoDeserialize(Dictionary<string, fsData> data, ref Ray2D model)
	{
		...
		fsResult fsResult = success + DeserializeMember<Vector2>(data, null, "origin", out value);
		model.origin = value;
		Vector2 value2 = model.direction;
		fsResult result = fsResult + DeserializeMember<Vector2>(data, null, "direction", out value2);
