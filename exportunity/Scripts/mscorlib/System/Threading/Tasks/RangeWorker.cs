	...
	internal unsafe bool FindNewWork(out long nFromInclusiveLocal, out long nToExclusiveLocal)
	{
		...
		do
		{
			...
			if (indexRange._bRangeFinished == 0)
			{
				...
				{
					Interlocked.CompareExchange(ref _indexRanges[_nCurrentIndexRange]._nSharedCurrentIndexOffset, new Box<long>(0L), null);
				}
				long num2;
				if (IntPtr.Size == 4 && _use32BitCurrentIndex)
				{
					fixed (long* value = &_indexRanges[_nCurrentIndexRange]._nSharedCurrentIndexOffset.Value)
					{
				...
				{
					num2 = Interlocked.Add(ref _indexRanges[_nCurrentIndexRange]._nSharedCurrentIndexOffset.Value, _nIncrementValue) - _nIncrementValue;
				}
