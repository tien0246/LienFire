	...
	private sealed class MultiElementAsyncLocalValueMap : IAsyncLocalValueMap
	{
		...
		public IAsyncLocalValueMap Set(IAsyncLocal key, object value)
		{
			for (int i = 0; i < _keyValues.Length; i++)
			{
				if (key == _keyValues[i].Key)
				{
					if (value != null)
					{
						...
						Array.Copy(_keyValues, 0, multiElementAsyncLocalValueMap._keyValues, 0, _keyValues.Length);
					...
					if (i != 0)
					{
						Array.Copy(_keyValues, 0, multiElementAsyncLocalValueMap2._keyValues, 0, i);
					}
			...
			if (_keyValues.Length < 16)
			{
				...
				Array.Copy(_keyValues, 0, multiElementAsyncLocalValueMap3._keyValues, 0, _keyValues.Length);
		...
		public IAsyncLocalValueMap Set(IAsyncLocal key, object value)
		{
			...
			if (flag)
			{
				if (count == 17)
				{
					...
					while (enumerator.MoveNext())
					{
						KeyValuePair<IAsyncLocal, object> current2 = enumerator.Current;
						if (key != current2.Key)
						{
				...
				while (enumerator.MoveNext())
				{
					KeyValuePair<IAsyncLocal, object> current3 = enumerator.Current;
					if (key != current3.Key)
					{
