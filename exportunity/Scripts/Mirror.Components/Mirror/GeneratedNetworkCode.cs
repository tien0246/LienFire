...
public static class GeneratedNetworkCode
{
	...
	public static void _Write_Mirror_002EDiscovery_002EServerResponse(NetworkWriter writer, ServerResponse value)
	{
		...
		writer.WriteLong(value.serverId);
	}

	public static void _Write_Mirror_002EStats(NetworkWriter writer, Stats value)
	{
		writer.WriteInt(value.connections);
		writer.WriteDouble(value.uptime);
		writer.WriteInt(value.configuredTickRate);
		writer.WriteInt(value.actualTickRate);
		writer.WriteLong(value.sentBytesPerSecond);
		writer.WriteLong(value.receiveBytesPerSecond);
	...
	public static Stats _Read_Mirror_002EStats(NetworkReader reader)
	{
		return new Stats
		{
			connections = reader.ReadInt(),
			uptime = reader.ReadDouble(),
			configuredTickRate = reader.ReadInt(),
			actualTickRate = reader.ReadInt(),
			sentBytesPerSecond = reader.ReadLong(),
			receiveBytesPerSecond = reader.ReadLong(),
