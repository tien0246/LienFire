...
public sealed class InvokeMember : MemberUnit
{
	...
	public override bool HandleDependencies()
	{
		...
		if (parameterNames == null && base.member.parameterTypes.Length == base.defaultValues.Count)
		{
			parameterNames = base.defaultValues.Select((KeyValuePair<string, object> defaultValue) => defaultValue.Key.Substring(1)).ToList();
		}
		return true;
	}
